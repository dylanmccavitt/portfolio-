/**
 * Project catalog for the player UI.
 *
 * Ported from the `P` array in the prototype
 * (`~/Projects/portfolio-redesign-prototypes/15-player-v4.html`), which is the
 * source of truth for copy, metrics, hues, seek values, and shot captions.
 * Per the issue #18 decision log, `lingoloop` is excluded — 13 of the
 * prototype's 14 projects ship here.
 */

export type StatusKind = 'dry' | 'live' | 'wip' | 'done';

/** Badge: machine variant + human label, mirroring the prototype's `status` tuple. */
export type ProjectStatus = readonly [kind: StatusKind, label: string];
...
export type ProjectLink = readonly [label: string, href: string];
export type Metric = readonly [value: string, label: string];
export type Credit = readonly [role: string, value: string];
...
export type Shot = ImageShot | SkeletonShot;
...
export const CATALOG: Project[] = [
  {
    id: 'agentic-trader', title: 'agentic-trader', sym: 'at',
    area: 'Agents & MCP', status: ['dry', 'Dry-run'], year: 2026, activity: 'live 06·23',
    hue: '#8b7cf6', wip: true, money: false,
    line: 'autonomous swing trader via Robinhood’s Agentic Trading MCP',
    seek: { from: 'dry-run', to: 'live jun 23', pct: 80 },
    links: [['GitHub ↗', LINKS.github]],
    metrics: [['15:45 ET', 'scheduled run, weekdays'], ['100%', 'decisions journaled in dry-run'], ['06·23', 'go-live date']],
    about: [
    ...
    stack: [['Runtime', 'claude code · launchd'], ['Broker', 'robinhood agentic mcp'], ['Signal', 'rsi(2) mean-reversion'], ['Status', 'dry-run → live 06·23']],
    shots: [
  ...
  {
    id: 'exit-manager', title: 'tastytrade-exit-manager', sym: 'em',
    area: 'Trading systems', status: ['live', 'Live'], year: 2026, activity: 'today',
    hue: '#50c878', wip: false, money: true,
    ...
    metrics: [['3', 'exit mechanisms: scale-out · trail · oco'], ['live', 'real money, every session'], ['0', 'positions it can open']],
    about: [
    ...
    stack: [['Language', 'python'], ['Broker', 'tastytrade api'], ['Orders', 'oco · trail · scale-out'], ['Repo', 'public']],
    shots: [
  ...
  {
    id: 'hood', title: 'hood', sym: 'hd',
    area: 'Trading systems', status: ['live', 'Active'], year: 2026, activity: 'today',
    hue: '#5da8e8', wip: false, money: false,
    line: 'portfolio tracker + high-water-mark withdrawal engine',
    seek: { from: 'tracking', to: 'active daily', pct: 100 },
    links: [['GitHub ↗', LINKS.github]],
    metrics: [['weekly', 'withdrawal cadence'], ['0', 'principal touched — gains only'], ['local', 'account data stays on-machine']],
    about: [
    ...
    stack: [['Language', 'typescript'], ['Shape', 'local-first dashboard'], ['Engine', 'high-water-mark rules'], ['Status', 'active daily']],
    shots: [
  ...
  {
    id: 'tradingview-mcp', title: 'tradingview-mcp', sym: 'tv',
    area: 'Agents & MCP', status: ['wip', 'WIP'], year: 2026, activity: '2d ago',
    hue: '#e6b450', wip: true, money: false,
    line: 'MCP server driving TradingView Desktop',
    seek: { from: 'scaffold', to: 'v1 charting', pct: 65 },
    links: [['GitHub ↗', LINKS.github]],
    metrics: [['5', 'stock universes configured'], ['full', 'pine compile round-trip'], ['v1', 'charting scope']],
    about: [
    ...
    stack: [['Language', 'typescript'], ['Protocol', 'mcp · cdp'], ['Target', 'tradingview desktop'], ['Status', 'v1 in progress']],
    shots: [
  ...
  {
    id: 'dog-log', title: 'dog log', sym: 'dg',
    area: 'iOS', status: ['live', 'App Store'], year: 2026, activity: 'v1.2',
    hue: '#ef8354', wip: false, money: false,
    line: 'multi-dog routine tracking · widgets · shared households',
    seek: { from: 'v1.2', to: 'review', pct: 95 },
    links: [['GitHub ↗', LINKS.github]],
    metrics: [['189', 'commits to v1.2'], ['2', 'storage modes: local + synced'], ['v1.2', 'in app store review']],
    about: [
    ...
    stack: [['Language', 'swift'], ['Storage', 'swiftdata · supabase'], ['Surface', 'app + widget'], ['Status', 'app store review']],
    shots: [
  ...
  {
    id: 'chore-ladder', title: 'chore ladder', sym: 'cl',
    area: 'iOS', status: ['wip', 'TestFlight'], year: 2026, activity: 'may',
    hue: '#50c878', wip: false, money: false,
    line: 'household chore boards with invites and a pro tier',
    seek: { from: 'build', to: 'testflight', pct: 90 },
    links: [['GitHub ↗', LINKS.github]],
    metrics: [['184', 'commits'], ['25+', 'unit test suites'], ['pro', 'subscription tier built']],
    about: [
    ...
    stack: [['Language', 'swift'], ['Backend', 'firebase'], ['Monetization', 'storekit subscription'], ['Status', 'testflight prep']],
    shots: [
  ...
  {
    id: 'homeserver', title: 'homeserver', sym: 'hs',
    area: 'Infrastructure', status: ['live', 'Online'], year: 2025, activity: '99.9%',
    hue: '#5da8e8', wip: false, money: false,
    line: 'declarative NixOS homelab, rebuilt from one flake',
    seek: { from: 'uptime', to: '99.9%', pct: 100 },
    links: [['nixos-dotfiles ↗', 'https://github.com/DylanMcCavitt/nixos-dotfiles'], ['homelab ↗', 'https://github.com/DylanMcCavitt/homelab']],
    metrics: [['3', 'nodes on the tailscale mesh'], ['99.9%', 'uptime'], ['1', 'flake to rebuild everything']],
    about: [
    ...
    stack: [['OS', 'nixos'], ['Network', 'tailscale · caddy'], ['Backup', 'restic → s3'], ['Observability', 'grafana · loki']],
    shots: [
      { src: `${SHOTS}/homelab/homepage.webp`, cap: 'service homepage' },
      { src: `${SHOTS}/homelab/proxmox.webp`, cap: 'proxmox cluster' },
      { src: `${SHOTS}/homelab/kuma.webp`, cap: 'uptime kuma monitors' },
      { src: `${SHOTS}/homelab/opnsense.webp`, cap: 'opnsense at the edge' },
    ],
  },
  {
    id: 'bellas-beads', title: "bella's beads", sym: 'bb',
    area: 'Shipped', status: ['done', 'Shipped'], year: 2025, activity: '2025',
    hue: '#d678b6', wip: false, money: false,
    ...
    metrics: [['400+', 'commits to handoff'], ['4', 'integrations: stripe · shippo · supabase · resend'], ['2', 'checkout flows: guest + account']],
    about: [
    ...
    stack: [['Frontend', 'react · typescript'], ['Backend', 'node · express'], ['Data', 'postgres / supabase'], ['Payments', 'stripe · shippo']],
    shots: [
      { src: `${SHOTS}/bella/landing.webp`, cap: 'storefront landing' },
      { src: `${SHOTS}/bella/product-page.webp`, cap: 'product page' },
      { src: `${SHOTS}/bella/stripe.webp`, cap: 'stripe checkout' },
      { src: `${SHOTS}/bella/admin-dash.webp`, cap: 'admin dashboard' },
    ],
  },
  {
    id: 'nhf', title: 'no hard feelings', sym: 'nh',
    area: 'Shipped', status: ['live', 'Live'], year: 2025, activity: 'live',
    hue: '#ef8354', wip: false, money: false,
    line: 'band site with a 3D-flippable album hero',
    seek: { from: 'build', to: 'live', pct: 100 },
    links: [['Live site ↗', 'https://nohardfeelings.app']],
    metrics: [['0', 'databases to maintain'], ['auto', 'show dates via google calendar'], ['live', 'nohardfeelings.app']],
    about: [
    ...
    stack: [['Framework', 'astro · react'], ['Style', 'tailwind · motion.js'], ['Shows', 'google calendar feed'], ['Status', 'live']],
    shots: [
      { src: `${SHOTS}/nohard/landing.webp`, cap: 'album-cover hero, front' },
      { src: `${SHOTS}/nohard/backcard.webp`, cap: 'flipped — band bios' },
      { src: `${SHOTS}/nohard/gcal-integration.webp`, cap: 'shows from google calendar' },
    ],
  },
  {
    id: 'work-orders', title: 'work orders', sym: 'wo',
    area: 'School', status: ['done', 'Shipped'], year: 2025, activity: 'team of 4',
    hue: '#5da8e8', wip: false, money: false,
    ...
    metrics: [['4', 'person team'], ['4', 'invoice states: draft · issued · paid · cancelled'], ['NYC', 'open-data contract seed']],
    about: [
    ...
    stack: [['Backend', 'express 5 · mongodb'], ['Templates', 'handlebars'], ['Auth', 'sessions · bcrypt'], ['Data', 'nyc open data']],
    shots: [
      { src: `${SHOTS}/work-order/work-order-landing.webp`, cap: 'landing' },
      { src: `${SHOTS}/work-order/woadmin.webp`, cap: 'admin dashboard' },
      { src: `${SHOTS}/work-order/invoice.webp`, cap: 'invoice lifecycle' },
    ],
  },
  {
    id: 'epl-ml', title: 'EPL match predictor', sym: 'ep',
    area: 'School', status: ['done', 'Shipped'], year: 2025, activity: '8 models',
    hue: '#50c878', wip: false, money: false,
    ...
    metrics: [['8', 'models, one split'], ['39', 'engineered features'], ['~99%', 'top accuracy (xgboost)'], ['20 yrs', 'of match data']],
    about: [
    ...
    stack: [['Language', 'python'], ['Models', '8-way comparison'], ['Winner', 'xgboost ~99%'], ['Data', 'kaggle · 20 yrs epl']],
    shots: [
      { src: `${SHOTS}/epl-ml-model/accuracy-comparison.webp`, cap: 'model accuracy comparison' },
      { src: `${SHOTS}/epl-ml-model/correlation-heatmap.webp`, cap: 'feature correlation heatmap' },
      { src: `${SHOTS}/epl-ml-model/xgboost.webp`, cap: 'xgboost results' },
    ],
  },
  {
    id: 'condor-study', title: '0DTE condor study', sym: '0d',
    area: 'Research', status: ['done', 'Retired'], year: 2025, activity: '2025',
    hue: '#969aa6', wip: false, money: false,
    line: 'no edge after costs — retired with a post-mortem',
    seek: { from: '2021', to: 'retired', pct: 100 },
    links: [['GitHub ↗', LINKS.github]],
    metrics: [['4 yrs', 'of walk-forward backtests'], ['0', 'edge after costs'], ['1', 'post-mortem published']],
    about: [
    ...
    stack: [['Platform', 'quantconnect · lean'], ['Underlying', 'spy 0dte'], ['Verdict', 'no edge after costs'], ['Status', 'retired']],
    shots: [
  ...
  {
    id: 'harness-arena', title: 'harness-arena', sym: 'ha',
    area: 'Research', status: ['done', 'Shelved'], year: 2026, activity: 'apr',
    hue: '#e6b450', wip: false, money: false,
    line: 'agent-harness evals through games',
    seek: { from: 'arena loop', to: 'shelved', pct: 55 },
    links: [['GitHub ↗', LINKS.github]],
    metrics: [['48', 'commits'], ['A/B', 'preset comparison design']],
    about: [
    ...
    stack: [['Language', 'typescript'], ['Shape', 'browser-first lab'], ['Status', 'shelved']],
    shots: [
...
export type PlaylistFilter = 'all' | 'wip' | 'money' | Area;
...
/** Sidebar model: library rows first, then the area playlists. */
export const PLAYLISTS: Playlist[] = [
  { filter: 'all', label: 'All work' },
  { filter: 'wip', label: 'Now building' },
  { filter: 'money', label: 'Live with real money' },
  { filter: 'Trading systems', label: 'Trading systems' },
  { filter: 'Agents & MCP', label: 'Agents & MCP' },
  { filter: 'iOS', label: 'iOS' },
  { filter: 'Shipped', label: 'Shipped' },
  { filter: 'School', label: 'School · Stevens' },
  { filter: 'Infrastructure', label: 'Infrastructure' },
...
export function filterProjects(filter: PlaylistFilter): Project[] {
  ...
  if (filter === 'money') return CATALOG.filter((p) => p.money);
  return CATALOG.filter((p) => p.area === filter);
}

export function playlistCount(filter: PlaylistFilter): number {
  return filterProjects(filter).length;
}
