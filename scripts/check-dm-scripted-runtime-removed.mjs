...
function dynamicCodeExecutionFailures(sourceFile) {
  ...
  let effectiveInvocationReferences = () => [];
  const reflectAdapterNames = new Set();
  ...
  const executionContextNode = (node) => {
  ...
  const recordFunctionDeclaration = (
    name,
    node,
    scope,
    position = node.pos,
    executionContext = executionContextNode(node),
  ) => {
    ...
    if (!declarations.some((declaration) => (
      ...
      declarations.push({
        ...
        position,
        executionContext,
        hoisted: ts.isFunctionDeclaration(node) && position === node.pos,
      });
    }
  ...
  const visibleFunctionNode = (path, reference) => {
    if (!path) return null;
    const declarations = functionDeclarations.get(path) ?? [];
    const declaration = declarations
      .flatMap((candidate) => {
        const { scope, position, hoisted, executionContext } = candidate;
        if (scopeContainsNode(scope, reference) && (hoisted || position < reference.pos)) {
          return [{ ...candidate, effectivePosition: position }];
        }
        if (!path.includes('.') || hoisted) return [];
        return effectiveInvocationReferences(executionContext, reference)
          .filter((invocation) => invocation.pos < reference.pos)
          .map((invocation) => ({ ...candidate, effectivePosition: invocation.pos }));
      })
      .sort((left, right) => (
        path.includes('.')
    ...
    return declarations.length === 0 ? functionNodes.get(path) ?? null : null;
  };
  ...
  const bindingDeclarationPaths = (name, prefix = [], paths = new Map()) => {
    ...
    if (ts.isObjectBindingPattern(name)) {
      ...
      for (const element of name.elements) {
        ...
        const key = element.propertyName
          ? propertyNameText(element.propertyName)
          : ts.isIdentifier(element.name) ? element.name.text : null;
        if (key !== null) bindingDeclarationPaths(element.name, [...prefix, key], paths);
      }
    }
    if (ts.isArrayBindingPattern(name)) {
      for (const [index, element] of name.elements.entries()) {
        if (!ts.isOmittedExpression(element)) {
          bindingDeclarationPaths(element.name, [...prefix, String(index)], paths);
        }
      }
    }
    return paths;
  ...
  walk(sourceFile, (node) => {
    ...
    if (ts.isParameter(node)) {
      for (const [binding, path] of bindingDeclarationPaths(node.name)) {
        ...
        declarations.push({ parameter: node, path, scope: node.parent });
        parameterDeclarations.set(binding, declarations);
      }
    }
    if (
  ...
  let reflectAdapterChanged = true;
  while (reflectAdapterChanged) {
    reflectAdapterChanged = false;
    for (const [name, declarations] of initializerDeclarations) {
      if (reflectAdapterNames.has(name)) continue;
      if (declarations.some(({ initializer }) => {
        const value = unwrapExpression(initializer);
        const path = staticPropertyPath(value, constBindings)?.join('.');
        return path === 'Reflect.apply'
          || path === 'Reflect.construct'
          || (ts.isIdentifier(value) && reflectAdapterNames.has(value.text));
      })) {
        reflectAdapterNames.add(name);
        reflectAdapterChanged = true;
      }
    }
    for (const [name, assignments] of assignmentDeclarations) {
      if (reflectAdapterNames.has(name)) continue;
      if (assignments.some(({ value: assigned }) => {
        const value = unwrapExpression(assigned);
        const path = staticPropertyPath(value, constBindings)?.join('.');
        return path === 'Reflect.apply'
          || path === 'Reflect.construct'
          || (ts.isIdentifier(value) && reflectAdapterNames.has(value.text));
  ...
  const visibleBindingIdentityCache = new WeakMap();
  const visibleBindingIdentity = (identifier) => {
    const cached = visibleBindingIdentityCache.get(identifier);
    ...
    const identity = candidates.sort((left, right) => (
      (left.scope.end - left.scope.pos) - (right.scope.end - right.scope.pos)
    ))[0] ?? {
  ...
  const reachingReferencesForIdentifier = (identifier) => {
    ...
    const references = effectiveInvocationReferences(identifierContext, identifier);
    return references.length > 0 ? references : [identifier];
  };
  ...
  const visibleAssignedValues = (identifier, reference = identifier) => {
    ...
    const declarations = (assignmentDeclarations.get(identifier.text) ?? [])
      .filter(({ identifier: target }) => visibleBindingIdentity(target).key === binding.key)
      .flatMap((declaration) => {
        const targetContext = executionContextNode(declaration.identifier);
        if (scopeContainsNode(targetContext, reference)) {
          return declaration.position < reference.pos ? [declaration] : [];
        }
        return effectiveInvocationReferences(targetContext, reference)
          .filter((invocation) => invocation.pos < reference.pos)
          .map((invocation) => ({ ...declaration, position: invocation.pos }));
      })
      .map((declaration) => ({
        ...declaration,
  ...
  let reachingIdentifierValuesCache = new WeakMap();
  ...
  let reachingNodesCache = new WeakMap();
  const resolveReachingNodes = (node, seen = new Set()) => {
    const value = unwrapExpression(node);
    if (!ts.isIdentifier(value)) return [value];
    if (seen.size === 0) {
      const cached = reachingNodesCache.get(value);
      if (cached) return cached;
    }
    const bindingKey = visibleBindingIdentity(value).key;
    if (seen.has(bindingKey)) return [value];
    const reaching = reachingIdentifierValues(value);
    if (reaching.length === 0) return [value];
    const nextSeen = new Set(seen).add(bindingKey);
    const resolved = reaching.flatMap((candidate) => resolveReachingNodes(candidate, nextSeen));
    if (seen.size === 0) reachingNodesCache.set(value, resolved);
    return resolved;
  };
  const reachingReflectAdapterPaths = (node, seen = new Set()) => {
    const value = unwrapExpression(node);
    const directPath = staticPropertyPath(value, constBindings)?.join('.');
    if (directPath === 'Reflect.apply' || directPath === 'Reflect.construct') {
      return [directPath];
    }
    if (!ts.isIdentifier(value) || !reflectAdapterNames.has(value.text)) return [];
    const bindingKey = visibleBindingIdentity(value).key;
    if (seen.has(bindingKey)) return [];
    const nextSeen = new Set(seen).add(bindingKey);
    return reachingIdentifierValues(value).flatMap((candidate) => (
      reachingReflectAdapterPaths(candidate, nextSeen)
    ));
  };
  ...
  const isScopedCallableIdentifier = (identifier) => {
    const callTimeReferences = reachingReferencesForIdentifier(identifier);
    if (callTimeReferences.some((reference) => reference !== identifier)) {
      const resolutionKey = `call-time:${visibleBindingIdentity(identifier).key}`;
      if (scopedCallableResolution.has(resolutionKey)) return false;
      scopedCallableResolution.add(resolutionKey);
      try {
        return reachingIdentifierValues(identifier).some((candidate) => {
          const value = unwrapExpression(candidate);
          if (ts.isFunctionExpression(value) || ts.isArrowFunction(value) || ts.isClassExpression(value)) {
            return true;
          }
          const path = staticPropertyPath(value, constBindings)?.join('.');
          ...
          return !ts.isIdentifier(value) && isTrackedCallableExpression(value);
        });
      } finally {
        scopedCallableResolution.delete(resolutionKey);
      }
    }
    ...
    const safeKill = assignments.find((assignment) => (
      assignment.operator === ts.SyntaxKind.EqualsToken
      && assignment.unconditional
      && assignment.position > declarationPosition
    ));
    if (safeKill) {
      const reachingAssignments = assignments.filter(({ position }) => position >= safeKill.position);
      return reachingAssignments.some((assignment) => {
  ...
  const assignmentTargetIdentifiers = (node, identifiers = []) => {
    ...
    if (ts.isIdentifier(target)) {
      ...
      assignmentTargetIdentifiers(target.expression, identifiers);
  ...
  const resolvedAssignmentBindingInitializers = (name, initializer, entries = []) => {
    const target = unwrapExpression(name);
    if (ts.isIdentifier(target)) {
      for (const value of resolveReachingNodes(initializer)) {
        entries.push({ name: target.text, identifier: target, initializer: value });
      }
      return entries;
    }
    if (ts.isArrayLiteralExpression(target)) {
      const expand = (value, seen = new Set()) => resolveReachingNodes(value).flatMap((candidate) => {
        ...
        return expression.elements.flatMap((element) => {
          ...
          return ts.isSpreadElement(element)
            ? expand(element.expression, nextSeen)
            : [element];
        });
      });
      const values = expand(initializer);
      for (const [index, element] of target.elements.entries()) {
        if (ts.isOmittedExpression(element) || ts.isSpreadElement(element)) continue;
        const selected = values[index];
        if (!selected || ts.isOmittedExpression(selected)) continue;
        resolvedAssignmentBindingInitializers(element, selected, entries);
      }
      return entries;
    }
    if (ts.isObjectLiteralExpression(target)) {
      for (const property of target.properties) {
        ...
        const key = propertyNameText(property.name);
        for (const value of resolveReachingNodes(initializer)) {
          for (const selected of literalMemberValues(value, key)) {
            resolvedAssignmentBindingInitializers(
              ts.isShorthandPropertyAssignment(property) ? property.name : property.initializer,
              selected,
              entries,
            );
  ...
  const invocationCandidates = (call) => {
    ...
    const reflectiveCandidates = [];
    let adapterPaths = [];
    if (ts.isCallExpression(call)) {
      const target = callArguments[0];
      adapterPaths = reachingReflectAdapterPaths(callee);
      for (const adapterPath of adapterPaths) {
        const listArgument = callArguments[adapterPath === 'Reflect.apply' ? 2 : 1];
        if (!target || !listArgument) continue;
        for (const list of resolveReachingNodes(listArgument)) {
          if (!ts.isArrayLiteralExpression(list)) continue;
          for (const resolvedTarget of resolveReachingNodes(target)) {
            let invocationTarget = resolvedTarget;
            if (
              adapterPath === 'Reflect.construct'
              && (ts.isClassExpression(resolvedTarget) || ts.isClassDeclaration(resolvedTarget))
            ) {
              invocationTarget = resolvedTarget.members.find(ts.isConstructorDeclaration)
                ?? resolvedTarget;
            }
            reflectiveCandidates.push({
    ...
    if (reflectiveCandidates.length > 0) return reflectiveCandidates;
    if (
      ts.isCallExpression(call)
      && ts.isIdentifier(callee)
      && reflectAdapterNames.has(callee.text)
      && adapterPaths.length === 0
    ) {
      return resolveReachingNodes(callee).map((resolved) => ({
        callee: resolved,
    ...
    if (ts.isIdentifier(callee)) {
      const resolveInvocationInitializers = (identifier, seen = new Set()) => {
        const bindingKey = visibleBindingIdentity(identifier).key;
        if (seen.has(bindingKey)) return [identifier];
        const values = reachingIdentifierValues(identifier);
        if (values.length === 0) {
          return callableFunctionsForCallee(identifier).length > 0 ? [identifier] : [];
        }
        return values.flatMap((initializer) => {
          const value = unwrapExpression(initializer);
          return ts.isIdentifier(value)
            ? resolveInvocationInitializers(value, new Set(seen).add(bindingKey))
            : [value];
        });
      };
      const initializers = resolveInvocationInitializers(callee);
      const invocationSources = (node, seenFunctions = new Set(), substitutions = new Map()) => {
        const value = node && unwrapExpression(node);
        if (!value) return { direct: [], bound: [] };
        if (ts.isIdentifier(value) && substitutions.has(value.text)) {
          const replacements = substitutions.get(value.text)
            .filter((replacement) => {
              const candidate = unwrapExpression(replacement);
              return !(ts.isIdentifier(candidate) && candidate.text === value.text);
            });
          return replacements.reduce((sources, replacement) => {
            const nested = invocationSources(replacement, seenFunctions, substitutions);
        ...
        if (ts.isConditionalExpression(value)) {
          const whenTrue = invocationSources(value.whenTrue, seenFunctions, substitutions);
          const whenFalse = invocationSources(value.whenFalse, seenFunctions, substitutions);
          return {
        ...
        if (
          ...
          const left = invocationSources(value.left, seenFunctions, substitutions);
          const right = invocationSources(value.right, seenFunctions, substitutions);
          return {
        ...
        const argumentsList = expandArguments(value.arguments).flatMap((argument) => {
          const candidate = unwrapExpression(argument);
          return ts.isIdentifier(candidate) && substitutions.has(candidate.text)
            ? substitutions.get(candidate.text)
            : [argument];
        });
        for (const fn of callableFunctionsForCallee(value.expression)) {
          ...
          for (const [index, parameter] of fn.parameters.entries()) {
            ...
            for (const [binding, selection] of parameterBindingPaths(parameter.name)) {
              ...
              for (const key of selection) {
                selected = selected.flatMap((candidate) => literalMemberValues(candidate, key));
              }
              if (selected.length > 0) nestedSubstitutions.set(binding, selected);
            }
          }
          for (const returned of functionReturnInitializers(fn)) {
            for (const selected of outputSelectedArguments(returned, fn, argumentsList)) {
              const nested = invocationSources(selected, nextSeen, nestedSubstitutions);
              sources.direct.push(...nested.direct);
              sources.bound.push(...nested.bound);
            }
            const nested = invocationSources(returned, nextSeen, nestedSubstitutions);
            sources.direct.push(...nested.direct);
            sources.bound.push(...nested.bound);
          }
        }
  ...
  const propagateCallableBinding = (name, initializer) => {
    ...
    if (ts.isArrayBindingPattern(target) || ts.isArrayLiteralExpression(target)) {
      const expandArrayCandidates = (node, seen = new Set()) => {
        const candidate = unwrapExpression(node);
        if (ts.isIdentifier(candidate)) {
          const bindingKey = visibleBindingIdentity(candidate).key;
          if (seen.has(bindingKey)) return [];
          const reaching = reachingIdentifierValues(candidate);
          if (reaching.length === 0) return [];
          const nextSeen = new Set(seen).add(bindingKey);
          return reaching.flatMap((value) => expandArrayCandidates(value, nextSeen));
        }
        if (!ts.isArrayLiteralExpression(candidate)) return [];
        let candidates = [[]];
        for (const element of candidate.elements) {
          if (ts.isOmittedExpression(element)) {
            candidates = candidates.map((values) => [...values, element]);
            continue;
          }
          ...
          const spreadCandidates = expandArrayCandidates(element.expression, seen);
          if (spreadCandidates.length === 0) return [];
          candidates = candidates.flatMap((values) => (
            spreadCandidates.map((spread) => [...values, ...spread])
          ));
        }
        return candidates;
      };
      const expandedCandidates = expandArrayCandidates(expression);
      for (const [index, element] of target.elements.entries()) {
        if (ts.isOmittedExpression(element)) continue;
        const restElement = ts.isSpreadElement(element)
          ...
          : ts.isBindingElement(element) && element.dotDotDotToken
            ? element.name
            : null;
        if (restElement && expandedCandidates.length > 0) {
          const restTarget = staticPropertyPath(restElement, constBindings)?.join('.');
          if (!restTarget) continue;
          for (const expandedValues of expandedCandidates) {
    ...
    if (ts.isObjectBindingPattern(target) || ts.isObjectLiteralExpression(target)) {
      for (const property of target.properties ?? target.elements) {
        if (ts.isBindingElement(property)) {
          ...
          const restTarget = staticPropertyPath(property.expression, constBindings)?.join('.');
          if (!restTarget) continue;
          const excluded = target.properties
            .filter((candidate) => !ts.isSpreadAssignment(candidate))
            .map((candidate) => propertyNameText(candidate.name));
          for (const entry of objectRestEntries(
            expression,
            `${objectRestPrefix}${JSON.stringify(excluded)}`,
          )) {
            changed = propagateCallableInitializer(
              `${restTarget}.${entry.path.join('.')}`,
              entry.value,
            ) || changed;
          }
        } else if (ts.isPropertyAssignment(property)) {
  ...
  walk(sourceFile, (node) => {
    ...
    for (const fn of callableFunctionsForCallee(node.right)) {
      recordFunctionDeclaration(
        target,
        fn,
        lexicalScopeNode(node),
        node.pos,
        executionContextNode(node),
      );
    }
  });
  walk(sourceFile, (node) => {
    ...
    if (
      ...
      && (
        ...
        || (
          ...
          && computedName(expression.argumentExpression) === 'bind'
        )
      )
    ) return;
    for (const invocation of invocationCandidates(node)) {
      for (const fn of callableFunctionsForCallee(invocation.callee)) {
        ...
        if (!references.includes(node)) references.push(node);
        invocationReferences.set(fn, references);
      }
    }
  });
  const invocationScale = Math.max(sourceFile.end + 1, 2);
  const effectiveInvocationCache = new Map();
  effectiveInvocationReferences = (fn, _reference, seen = new Set()) => {
    if (!fn || seen.has(fn)) return [];
    if (seen.size === 0 && effectiveInvocationCache.has(fn)) {
      return effectiveInvocationCache.get(fn);
    }
    const nextSeen = new Set(seen).add(fn);
    const references = (invocationReferences.get(fn) ?? []).flatMap((invocation) => {
      const caller = executionContextNode(invocation);
      if (ts.isSourceFile(caller)) return [invocation];
      const outerReferences = effectiveInvocationReferences(caller, invocation, nextSeen);
      if (
        outerReferences.length === 0
        && (
          ts.isSourceFile(executionContextNode(caller.parent))
          || ts.isMethodDeclaration(caller)
          || ts.isGetAccessorDeclaration(caller)
          || ts.isSetAccessorDeclaration(caller)
          || (
            (ts.isFunctionExpression(caller) || ts.isArrowFunction(caller))
            && (
              ts.isPropertyAssignment(caller.parent)
              || ts.isMethodDeclaration(caller.parent)
            )
          )
        )
      ) return [invocation];
      return outerReferences.map((outer) => ({
        ...outer,
        pos: outer.pos + (invocation.pos / (invocationScale ** nextSeen.size)),
      }));
    });
    if (seen.size === 0) effectiveInvocationCache.set(fn, references);
    return references;
  };
  reachingIdentifierValuesCache = new WeakMap();
  reachingNodesCache = new WeakMap();
  ...
  while (callableChanged) {
    ...
    walk(sourceFile, (node) => {
          ...
          for (const [index, parameter] of fn.parameters.entries()) {
            ...
            for (const [binding, selection] of parameterBindingPaths(parameter.name)) {
              ...
              if (
                ...
                scopedCallableParameters.add(parameterKey);
                callableChanged = true;
              }
            }
          }
      ...
      if (
        ...
        const memberTarget = ts.isPropertyAccessExpression(assignedTarget)
          || ts.isElementAccessExpression(assignedTarget);
        const invokedMemberEffect = memberTarget
          && effectiveInvocationReferences(executionContextNode(node), node).length > 0;
        const crossesExecutionContext = localTargets.size !== targetIdentifiers.length
          && !invokedMemberEffect;
        if (crossesExecutionContext) {
          for (const binding of resolvedAssignmentBindingInitializers(node.left, node.right)) {
        ...
        activeCallableDeclarations = new Map();
        if (crossesExecutionContext) return;
        const target = staticPropertyPath(node.left, constBindings)?.join('.');
  ...
  walk(sourceFile, (node) => {
    ...
    if (
      ts.isCallExpression(node)
      && (
        reachingReflectAdapterPaths(node.expression).includes('Reflect.construct')
      )
    ) unsafe = true;
  });
  return unsafe
...
function trustedPrimitiveMutationFailures(sourceFile) {
  ...
  const aliasRisk = (path) => {
    if (!path) return 0;
    if (
      path[0] === 'UnknownIntrinsic'
      || path[0] === 'z'
      || path[0] === 'globalThis'
      || GOVERNED_INTRINSIC_PROTOTYPES.has(path[0])
      || path.at(-1) === '$descriptors'
  ...
  const storeAlias = (name, path) => {
    ...
    if (existing?.join('.') === path.join('.')) return false;
    if (existing && aliasRisk(existing) >= aliasRisk(path)) return false;
...
const EXPECTED_V2_PROSE_HELPER_BODIES = new Map([
  ...
  ['isSafeV2ProseOnlyFinishReason', "{returnreason==='stop';}"],
