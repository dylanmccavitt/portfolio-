...
| DM live-model release capture | `npm run dm:eval:release -- --capture-release` | Paid fixed Luna/Grok matrix, three runs per case; emits an explicit no-winner report with exact candidate digests |
| DM release qualification | `npm run dm:eval:release -- --release-report <captured.json> --selection-evidence <sanitized.json>` | Provider-free fail-closed aggregate qualification of exact captured runs; canned answers and missing or stale blinded evidence cannot satisfy it |
