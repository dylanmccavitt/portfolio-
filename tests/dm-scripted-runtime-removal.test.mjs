...
test('rejects dynamic evaluation and function construction in the governed runtime', async (t) => {
  ...
  const mutations = [
    ...
    `const fn = () => {}; const identity = (value: any) => value; const args = [fn]; const nested = [...args]; const callable = identity.apply(null, nested); const key = ['con', 'structor'].join(''); let C: any; [C] = [callable[key]]; C(${JSON.stringify(hiddenWrite)})();`,
    `const fn = () => {}; const name = getPublicToolName(); const entry = [name, fn]; const entries = [entry]; const holder: any = Object.fromEntries(entries); const callable = holder[name]; const key = ['con', 'structor'].join(''); let C: any; [C] = [callable[key]]; C(${JSON.stringify(hiddenWrite)})();`,
    `const fn = () => {}; class Holder { expose() { return fn; } } const identity = (value: any) => { let alias: any; alias = value; return alias; }; const C = identity(Holder); const callable = new C().expose(); const key = ['con', 'structor'].join(''); let Constructor: any; [Constructor] = [callable[key]]; Constructor(${JSON.stringify(hiddenWrite)})();`,
    `const fn = () => {}; let callable: any = { safe: true }; callable = fn; const key = ['con', 'structor'].join(''); let C: any; [C] = [callable[key]]; C(${JSON.stringify(hiddenWrite)})();`,
    `const fn = () => {}; const identity = (value: any) => value; const bound = identity.bind(null, fn); const callable = bound(); const key = ['con', 'structor'].join(''); let C: any; [C] = [callable[key]]; C(${JSON.stringify(hiddenWrite)})();`,
    `const fn = () => {}; class Holder { expose() { return fn; } } const identity = (value: any) => value; const delegate = (value: any) => identity(value); const C = delegate(Holder); const callable = new C().expose(); const key = ['con', 'structor'].join(''); let Constructor: any; [Constructor] = [callable[key]]; Constructor(${JSON.stringify(hiddenWrite)})();`,
    `const fn = () => {}; let callable: any = { safe: true }; [callable] = [fn]; const key = ['con', 'structor'].join(''); let C: any; [C] = [callable[key]]; C(${JSON.stringify(hiddenWrite)})();`,
    `const fn = () => {}; const identity = (value: any) => value; let bound: any; bound = identity.bind(null, fn); const callable = bound(); const key = ['con', 'structor'].join(''); let C: any; [C] = [callable[key]]; C(${JSON.stringify(hiddenWrite)})();`,
    `const fn = () => {}; class Holder { expose() { return fn; } } class Other {} const choose = (value: any) => getPublicToolName() ? value : Other; const C = choose(Holder); const callable = new C().expose(); const key = ['con', 'structor'].join(''); let Constructor: any; [Constructor] = [callable[key]]; Constructor(${JSON.stringify(hiddenWrite)})();`,
    ...
    `const fn = () => {}; function exploit(callable: any) { const key = ['con', 'structor'].join(''); let C: any; [C] = [callable[key]]; C(${JSON.stringify(hiddenWrite)})(); } exploit(fn);`,
    `const fn = () => {}; const key = ['con', 'structor'].join(''); try { throw fn; } catch (callable) { let C: any; [C] = [callable[key]]; C(${JSON.stringify(hiddenWrite)})(); }`,
    ...
    `const fn = () => {}; const key = ['con', 'structor'].join(''); const dangerous = (callable: any) => { const C = callable[key]; C(${JSON.stringify(hiddenWrite)})(); }; let target: any = (_value: unknown) => undefined; if (getPublicToolName()) { target = dangerous; } target(fn);`,
    `const fn = () => {}; const key = ['con', 'structor'].join(''); const dangerous = (callable: any) => { const C = callable[key]; C(${JSON.stringify(hiddenWrite)})(); }; let target: any = (_value: unknown) => undefined; for (const selected of [dangerous]) { target = selected; } target(fn);`,
    `const fn = () => {}; const key = ['con', 'structor'].join(''); const dangerous = (callable: any) => { const C = callable[key]; C(${JSON.stringify(hiddenWrite)})(); }; let target: any = (_value: unknown) => undefined; switch (getPublicToolName()) { case 'dangerous': { target = dangerous; break; } } target(fn);`,
    `const fn = () => {}; const key = ['con', 'structor'].join(''); const dangerous = (callable: any) => { const C = callable[key]; C(${JSON.stringify(hiddenWrite)})(); }; const apply = Reflect.apply; apply(dangerous, null, [fn]);`,
    `const fn = () => {}; const key = ['con', 'structor'].join(''); const Dangerous = class { constructor(callable: any) { const C = callable[key]; C(${JSON.stringify(hiddenWrite)})(); } }; const construct = Reflect.construct; construct(Dangerous, [fn]);`,
    `const fn = () => {}; const key = ['con', 'structor'].join(''); class Dangerous { constructor(callable: any) { const C = callable[key]; C(${JSON.stringify(hiddenWrite)})(); } } new Dangerous(fn);`,
    ...
    `const fn = () => {}; const key = ['con', 'structor'].join(''); const dangerous = (callable: any) => { const C = callable[key]; C(${JSON.stringify(hiddenWrite)})(); }; const safe = (_value: unknown) => undefined; let target: any = safe; function invoke() { target(fn); } target = dangerous; invoke();`,
    `const fn = () => {}; const key = ['con', 'structor'].join(''); const dangerous = (callable: any) => { const C = callable[key]; C(${JSON.stringify(hiddenWrite)})(); }; const safe = (..._args: any[]) => undefined; let apply: any = safe; let target: any = safe; let args: any[] = []; function invoke() { apply(target, null, args); } apply = Reflect.apply; target = dangerous; args = [fn]; invoke();`,
    `const fn = () => {}; const key = ['con', 'structor'].join(''); const dangerous = (callable: any) => { const C = callable[key]; C(${JSON.stringify(hiddenWrite)})(); }; let source: any[] = []; function invoke() { let targets: any[]; [...targets] = source; targets[0](fn); } source = [dangerous]; invoke();`,
    `const fn = () => {}; const key = ['con', 'structor'].join(''); const bad = { exploit(callable: any) { const C = callable[key]; C(${JSON.stringify(hiddenWrite)})(); } }; let outer: any = {}; function invoke() { let local: any = {}; [outer, local] = [{}, bad]; local.exploit(fn); } invoke();`,
    `const fn = () => {}; const key = ['con', 'structor'].join(''); const dangerous = (callable: any) => { const C = callable[key]; C(${JSON.stringify(hiddenWrite)})(); }; const safe = (_value: unknown) => undefined; let target: any = safe; function inner() { target(fn); } function outer() { inner.call(null); } const callback = outer.bind(null); target = dangerous; callback();`,
    `const fn = () => {}; const key = ['con', 'structor'].join(''); const dangerous = (callable: any) => { const C = callable[key]; C(${JSON.stringify(hiddenWrite)})(); }; const safe = (_value: unknown) => undefined; let target: any = safe; function inner() { target(fn); } function outer() { inner.apply(null, []); } const alias = outer; target = dangerous; alias();`,
    `const fn = () => {}; const key = ['con', 'structor'].join(''); const dangerous = (callable: any) => { const C = callable[key]; C(${JSON.stringify(hiddenWrite)})(); }; const safe = (_value: unknown) => undefined; let target: any = safe; function setTarget() { target = dangerous; } function invoke() { target(fn); } setTarget(); invoke();`,
    `const fn = () => {}; const key = ['con', 'structor'].join(''); const bad = { exploit(callable: any) { const C = callable[key]; C(${JSON.stringify(hiddenWrite)})(); } }; const values = [{}, bad]; const pair = [...values]; let outer: any = {}; function invoke() { let local: any = {}; [outer, local] = pair; local.exploit(fn); } invoke();`,
    `const fn = () => {}; const key = ['con', 'structor'].join(''); const dangerous = (callable: any) => { const C = callable[key]; C(${JSON.stringify(hiddenWrite)})(); }; const holder: any = { exploit(_value: unknown) {} }; function install() { holder['exploit'] = dangerous; } install(); holder.exploit(fn);`,
  ];
  for (const [index, mutation] of mutations.entries()) {
    await t.test(String(index), () => {
      const mutated = runtime.replace(
...
test('keeps unrelated callable names from tainting scoped safe dynamic reads', async () => {
  const runtime = await liveRuntimeSource();
  const mutated = runtime.replace(
    '        finalizationResult ??= limitedResult(finalizationAttempts > 0);',
    "        function unrelated() { const value = () => 'callable'; void value; } { const value: any = { safe: true }; const safeKey = getPublicToolName(); void value[safeKey]; } void unrelated;\n        finalizationResult ??= limitedResult(finalizationAttempts > 0);",
  );
  assert.ok(!finalizationBoundaryFailures(mutated).includes(
    'src/lib/dm/runtime.ts: governed runtime source must not use dynamic code evaluation or function construction',
  ));
...
test('keeps unrelated callable names from tainting safe destructured bindings', async () => {
  const runtime = await liveRuntimeSource();
  const mutated = runtime.replace(
    '        finalizationResult ??= limitedResult(finalizationAttempts > 0);',
...
test('keeps unrelated callable names from tainting safe call-result destructuring', async () => {
  const runtime = await liveRuntimeSource();
  const mutated = runtime.replace(
    '        finalizationResult ??= limitedResult(finalizationAttempts > 0);',
...
test('keeps unrelated callable names from tainting delegated safe factory results', async () => {
  const runtime = await liveRuntimeSource();
  const mutated = runtime.replace(
    '        finalizationResult ??= limitedResult(finalizationAttempts > 0);',
...
test('keeps unrelated callable names from tainting safe parameter reads', async () => {
  const runtime = await liveRuntimeSource();
  const mutated = runtime.replace(
    '        finalizationResult ??= limitedResult(finalizationAttempts > 0);',
    "        function unrelated() { const value = () => 'callable'; void value; } function inspect(value: any, safeKey: string) { void value[safeKey]; } inspect({ safe: true }, getPublicToolName()); void unrelated;\n        finalizationResult ??= limitedResult(finalizationAttempts > 0);",
  );
  assert.ok(!finalizationBoundaryFailures(mutated).includes(
    'src/lib/dm/runtime.ts: governed runtime source must not use dynamic code evaluation or function construction',
  ));
...
test('keeps unrelated callable names from tainting safe catch bindings', async () => {
  const runtime = await liveRuntimeSource();
  const mutated = runtime.replace(
    '        finalizationResult ??= limitedResult(finalizationAttempts > 0);',
...
test('uses a definitely executed nested-block safe assignment at a later outer read', async () => {
  const runtime = await liveRuntimeSource();
  const mutated = runtime.replace(
    '        finalizationResult ??= limitedResult(finalizationAttempts > 0);',
    "        const fn = () => 'callable'; let value: any = fn; { value = { safe: true }; } const safeKey = getPublicToolName(); void value[safeKey];\n        finalizationResult ??= limitedResult(finalizationAttempts > 0);",
...
test('uses call-time reaching state for captured helper bindings', async () => {
  const runtime = await liveRuntimeSource();
  const mutated = runtime.replace(
    '        finalizationResult ??= limitedResult(finalizationAttempts > 0);',
    "        const fn = () => 'callable'; const key = ['con', 'structor'].join(''); const dangerous = (callable: any) => { void callable[key]; }; const safe = (..._args: any[]) => undefined; let target: any = dangerous; function invokeTarget() { target(fn); } target = safe; invokeTarget(); let apply: any = Reflect.apply; let reflectTarget: any = dangerous; let args: any[] = [fn]; function invokeReflect() { apply(reflectTarget, null, args); } apply = safe; reflectTarget = safe; args = []; invokeReflect(); let source: any[] = [dangerous]; function invokeRest() { let targets: any[]; [...targets] = source; const selected: any = targets[0]; void selected?.[key]; } source = []; invokeRest();\n        finalizationResult ??= limitedResult(finalizationAttempts > 0);",
  );
  assert.ok(!finalizationBoundaryFailures(mutated).includes(
    'src/lib/dm/runtime.ts: governed runtime source must not use dynamic code evaluation or function construction',
  ));
...
test('uses transitive call-time state and invoked setter effects without leaking uninvoked member writes', async () => {
  const runtime = await liveRuntimeSource();
  const mutated = runtime.replace(
    '        finalizationResult ??= limitedResult(finalizationAttempts > 0);',
    "        const fn = () => 'callable'; const key = ['con', 'structor'].join(''); const dangerous = (callable: any) => { void callable[key]; }; const safe = (_value: unknown) => undefined; let target: any = dangerous; function inner() { target(fn); } function outer() { inner.call(null); } const callback = outer.bind(null); target = safe; callback(); let sibling: any = dangerous; function setSibling() { sibling = safe; } function invokeSibling() { sibling(fn); } setSibling(); invokeSibling(); const holder: any = { exploit: dangerous, element: dangerous }; function setMembers() { holder.exploit = safe; holder['element'] = safe; } setMembers(); holder.exploit(fn); holder.element(fn); function neverCalled() { holder.exploit = dangerous; holder['element'] = dangerous; } void neverCalled; holder.exploit(fn); holder.element(fn);\n        finalizationResult ??= limitedResult(finalizationAttempts > 0);",
  );
  assert.ok(!finalizationBoundaryFailures(mutated).includes(
    'src/lib/dm/runtime.ts: governed runtime source must not use dynamic code evaluation or function construction',
  ));
...
test('ignores uninitialized outer callable writes inside uninvoked helpers', async () => {
  const runtime = await liveRuntimeSource();
  const mutated = runtime.replace(
    '        finalizationResult ??= limitedResult(finalizationAttempts > 0);',
    "        const fn = () => 'callable'; let value: any; function neverCalled() { value = fn; } void neverCalled; const safeKey = getPublicToolName(); void value?.[safeKey];\n        finalizationResult ??= limitedResult(finalizationAttempts > 0);",
  );
  assert.ok(!finalizationBoundaryFailures(mutated).includes(
    'src/lib/dm/runtime.ts: governed runtime source must not use dynamic code evaluation or function construction',
  ));
...
test('ignores class instances assigned only inside uninvoked helpers', async () => {
  const runtime = await liveRuntimeSource();
  const mutated = runtime.replace(
    '        finalizationResult ??= limitedResult(finalizationAttempts > 0);',
    "        const fn = () => 'callable'; const key = ['con', 'structor'].join(''); class Dangerous { exploit(callable: any) { void callable[key]; } } let holder: any = { exploit(_value: unknown) {} }; function neverCalled() { holder = new Dangerous(); } void neverCalled; holder.exploit(fn);\n        finalizationResult ??= limitedResult(finalizationAttempts > 0);",
  );
  assert.ok(!finalizationBoundaryFailures(mutated).includes(
    'src/lib/dm/runtime.ts: governed runtime source must not use dynamic code evaluation or function construction',
  ));
...
test('keeps switch-scoped run shadowing from suppressing authenticated run governance', async () => {
  const runtime = await liveRuntimeSource();
  const helperStart = `function createRuntimePublicTools(
  run: PublicAgentToolRun,
  artifacts: RunArtifacts,
  ...
  const mutated = runtime.replace(
    helperStart,
    `${helperStart}\n  switch (getPublicToolName()) { case 'never': let run = { evidenceLedger: { safe: true } }; void run; break; }\n  run.evidenceLedger = { ...run.evidenceLedger };`,
  );
  assert.ok(finalizationBoundaryFailures(mutated).includes(
    'src/lib/dm/runtime.ts: governed v2 dependency publicRun.evidenceLedger must not be replaced or redefined',
  ));
...
test('keeps class-static-block run shadowing from suppressing authenticated run governance', async () => {
  const runtime = await liveRuntimeSource();
  const helperStart = `function createRuntimePublicTools(
  run: PublicAgentToolRun,
  artifacts: RunArtifacts,
  ...
  const mutated = runtime.replace(
    helperStart,
    `${helperStart}\n  class Shadow { static { const run = { evidenceLedger: { safe: true } }; void run; } }\n  void Shadow;\n  run.evidenceLedger = { ...run.evidenceLedger };`,
  );
  assert.ok(finalizationBoundaryFailures(mutated).includes(
    'src/lib/dm/runtime.ts: governed v2 dependency publicRun.evidenceLedger must not be replaced or redefined',
  ));
