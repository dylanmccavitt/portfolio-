...
import { ArrowLeft, ArrowUpRight, Hammer, Wrench, X } from "lucide-react";
...
const PICKER = [
  ["/html/signal-tear", "01"],
  ["/html/signal-current", "02"],
  ["/html/signal-fracture", "03"],
  ["/html/signal-mend", "04"],
  ["/html/signal-frost", "05"],
  ["/html/signal-ember", "06"],
];
...
/* The locked layout: sticky nav, anchored sections, scrollspy. */
function SiteLayout({ shatterRef, onBella, onDm }) {
  const [current, setCurrent] = useState("about");

  useEffect(() => {
    const sections = DESTINATIONS
      .map((d) => document.getElementById(`site-${d.id}`))
      .filter(Boolean);
    const io = new IntersectionObserver(
      (entries) => {
        ...
        if (visible) setCurrent(visible.target.dataset.section);
      },
  ...
  const go = (id) => {
    document.getElementById(`site-${id}`)?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  ...
  return (
    <div className="frost-site">
      ...
      <section className="frost-site-section frost-site-hero" id="site-about" data-section="about">
        <p className="frost-kicker">Software engineer · New York City · {PROFILE.status.toLowerCase()}</p>
        <AboutBlock />
      </section>

      <section className="frost-site-section" id="site-work" data-section="work">
        <h2>Work</h2>
        <p className="frost-kicker">Four projects · shipped and building</p>
        <WorkRows onBella={onBella} />
      </section>

      <section className="frost-site-section" id="site-journey" data-section="journey">
        <h2>Journey</h2>
        <p className="frost-kicker">2019 — now</p>
        <JourneyRows />
      </section>

      <section className="frost-site-section" id="site-contact" data-section="contact">
        <h2>Contact</h2>
  ...
  return (
    <div className="frost-backdrop" onMouseDown={onClose}>
      ...
      >
        ...
        <p>
          DM answers only from Dylan&rsquo;s published portfolio sources. It is
...
function CanvasStatus({ effectMode }) {
  ...
  const tier = effectMode === "off"
    ? "effect disabled"
    : effectMode === "snapshot"
      ? "snapshot renderer (forced)"
      : native
  ...
  return (
    <span>
      Canvas UI / Shatter (inverted) / {tier}
    </span>
  );
}

export function FrostRoute({ prototype, navigate }) {
  ...
  return (
    <main className="frost">
      <Wrapper {...wrapperProps}>
        <div className="frost-page">
          <nav className="frost-topbar" aria-label="Prototype options">
            <a href="/" onClick={route(navigate, "/")}>
              <ArrowLeft size={13} /> All prototypes
            </a>
            <div className="frost-picker">
              {PICKER.map(([path, number]) => (
                <a
                  key={path}
                  href={path}
                  onClick={route(navigate, path)}
                  aria-current={path === prototype.path ? "page" : undefined}
                >
                  {number}
                </a>
              ))}
          ...
          <footer className="frost-footer">
            <span>{prototype.instruction} Shatter and Repair act on the whole page.</span>
            <CanvasStatus effectMode={effectMode} />
          </footer>
        </div>
      </Wrapper>
