...
void main () {
  ...
  float ink = smoothstep(g0, g1, dot(abs(base.rgb - uBg), lw));

  vec3 warmInk = base.rgb * vec3(0.90, 0.85, 0.80);
  vec3 col = mix(base.rgb, warmInk, e * ink * clamp(uWarmth, 0.0, 1.0));
  col += vec3(0.055, 0.042, 0.022) * e * (1.0 - ink) * clamp(uGlow, 0.0, 1.0);

  float n = hash(floor(gl_FragCoord.xy / 2.0));
  col += (n - 0.5) * uGrain * (0.5 + 0.5 * e);
...
export function createLamp(elements, options = {}) {
  ...
  function compile(type, text) {
    ...
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
      console.error("Lamp shader error:", gl.getShaderInfoLog(shader));
    }
    return shader;
  }
  ...
  function render() {
    ...
    gl.uniform1f(uniforms.uWarmth, config.warmth);
...
export function Lamp({
  ...
  useEffect(() => {
    ...
    instanceRef.current = createLamp({ source, content, output }, initialOptions);
